...
class Repository(IRepository):
	...
	def get_by_id_external_gas_reduction(self, id_external_gas_reduction) -> ImprovementPlan:
		try:
			query, projection = q.get_by_id_external_gas_reduction_query(id_external_gas_reduction)
			improvement_plan_data = self.db["improvement_plan"].find_one(query, projection)
			if not improvement_plan_data:
				raise ValueError(f"Improvement plan with id_external_gas_reduction {id_external_gas_reduction} not found.")
...
def improvement_plan_from_data(data: dict) -> ImprovementPlan:
	return ImprovementPlan(
		...
		id_external_gas_reduction=data.get("id_external_gas_reduction"),
